...
//		System.out.printf("ECDHServerKeyExchange/Constructor1: signatureAndHashAlgorithm %s\n", this.signatureAndHashAlgorithm.toString());
		...
		try {
			...
			signature.initSign(serverPrivateKey, new CustomSecureRandom());

			updateSignature(signature, clientRandom, serverRandom);

			signatureEncoded = signature.sign();

//			System.out.printf("ECDHServerKeyExchange/Constructor1: signatureEncoded\n%s\n",
//					org.eclipse.californium.scandium.util.Utils
//							.asHexCut20(signatureEncoded, 0, signatureEncoded.length));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	 ...
	 */
	public ECDHServerKeyExchange(SignatureAndHashAlgorithm signatureAndHashAlgorithm, int curveId, byte[] pointEncoded,
...
//		System.out.printf("ECDHServerKeyExchange/Constructor2: signatureEncoded\n%s\n",
//			org.eclipse.californium.scandium.util.Utils.asHexCut20(signatureEncoded, 0, signatureEncoded.length));
	}
		...
		// TODO add support for other curve types
			...
			case NAMED_CURVE:
				...
				if (signatureEncoded != null) {
					// according to
					// http://tools.ietf.org/html/rfc5246#section-A.7 the
...
//		System.out.printf("ECDHServerKeyExchange/fragmentToByteArray: data\n%s\n", org.eclipse.californium.scandium.util.Utils.asHexCut20(data, 0, data.length));

		return data;
	}

	public static HandshakeMessage fromByteArray(byte[] byteArray) throws HandshakeException {
//		System.out.printf("ECDHServerKeyExchange/fromByteArray: data\n%s\n", org.eclipse.californium.scandium.util.Utils.asHexCut20(byteArray, 0, byteArray.length));
		...
		switch (curveType) {
			...
			case NAMED_CURVE:
				...
				SignatureAndHashAlgorithm signAndHash = new SignatureAndHashAlgorithm(HashAlgorithm.SHA256,
	...
	public int getMessageLength() {
		...
		switch (curveType) {
			...
			case NAMED_CURVE:
				// the signature length field uses 2 bytes, if a signature
				// available
...
//			System.out.printf("ECDHServerKeyExchange/verifySignature: algorithm %s %s\n", signatureAndHashAlgorithm.toString(), serverPublicKey.getClass().toString());
//			
//			byte[] keyEncoded = serverPublicKey.getEncoded();
//			
//			System.out.printf("ServerPublicKey encoded\n%s\n",
//					org.eclipse.californium.scandium.util.Utils
//					.asHexCut20(keyEncoded, 0, keyEncoded.length));
//			
//			System.out.printf("Signature encoded\n%s\n",
//					org.eclipse.californium.scandium.util.Utils
//							.asHexCut20(signatureEncoded, 0, signatureEncoded.length));
	...
	private void updateSignature(Signature signature, Random clientRandom, Random serverRandom)
			throws SignatureException {
		byte[] dataClient = clientRandom.getRandomBytes();
		byte[] dataServer = serverRandom.getRandomBytes();

		signature.update(dataClient);
		signature.update(dataServer);
...
//				System.out
//						.printf("ECDHServerKeyExchange/updateSignature: dataClient\n%s\ndataServer\n%s\nNAMED_CURVE %x curveIdHi %x curvIdLo %x lengthPointEncoded %x\npointEncoded\n%s\n",
//								org.eclipse.californium.scandium.util.Utils
//										.asHexCut20(dataClient, 0, dataClient.length),
//								org.eclipse.californium.scandium.util.Utils
//										.asHexCut20(dataServer, 0, dataServer.length), (byte) NAMED_CURVE,
//								(byte) (curveId >> 8), (byte) curveId, (byte) pointEncoded.length,
//								org.eclipse.californium.scandium.util.Utils.asHexCut20(pointEncoded, 0,
//										pointEncoded.length));
	...
	private static void addParameterSpec(int namedCurveId, String p, String a, String b, String x, String y, String n,
			int h) {
	...
	static {
		...
		// secp256k1
		addParameterSpec(22, "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F",
		...
		addParameterSpec(23, "FFFFFFFF00000001000000000000000000000000FFFFFFFFFFFFFFFFFFFFFFFF",
